...
NS_LOG_COMPONENT_DEFINE ("Fixed-UAV");
...
public:
   ...
   * \return none
   */
  void Install (NodeContainer & c, // Vehicle nodes
                NodeContainer & u, // RSU nodes
                NodeContainer & uav, // UAV node container
                NetDeviceContainer & d, // Vehicle NetDevices
                NetDeviceContainer & ud, // RSU NetDevices
                NetDeviceContainer & udsr, // RSU NetDevices secondary
                NetDeviceContainer & uavd, // UAV netdevice
                NetDeviceContainer & uavdsr, // UAV netdevice secondary
                Ipv4InterfaceContainer & i, // Vehicle IP Interfaces,
                Ipv4InterfaceContainer & ui, // RSU IP Interfaces,
                Ipv4InterfaceContainer & uisr, // RSU IP Interfaces secondary,
                Ipv4InterfaceContainer & uavi, // UAV IP Interface
                Ipv4InterfaceContainer & uavisr, // UAV IP Interface secondary
                double totalTime,
                int protocol,
                uint32_t nSinks,
                int routingTables);
...
private:
   ...
   * \return none
   */
  void SetupRoutingProtocol (NodeContainer & c, NodeContainer & u, NodeContainer & uav); // u is for RSU nodes, v is for rsu nodes, uav for UAV nodes
  ...
  void AssignIpAddresses (NetDeviceContainer & d, // vehicle
                          ...
                          NetDeviceContainer & udsr, // rsu secondary
                          Ipv4InterfaceContainer & ui, // rsu Ip interface
                          Ipv4InterfaceContainer & uisr, // rsu Ip interface secondary
                          NetDeviceContainer & uavd, // UAV netdevice
                          NetDeviceContainer & uavdsr, // UAV secondary netdevice
                          Ipv4InterfaceContainer & uavi, // uav ip interface
                          Ipv4InterfaceContainer & uavisr); // uav secondary ip interface
...
void
RoutingHelper::Install (NodeContainer & c, // vehicular node
                        NodeContainer & u, // Added for RSU node
                        NodeContainer & uav, // UAV Node
                        NetDeviceContainer & d, // netdevice for vehicle
                        NetDeviceContainer & ud, // Added for RSU NetDevice
                        NetDeviceContainer & udsr, // Added for RSU NetDevice secondary
                        NetDeviceContainer & uavd, // UAV NetDevice
                        NetDeviceContainer & uavdsr, // UAV NetDevice secondary
                        Ipv4InterfaceContainer & i,
                        Ipv4InterfaceContainer & ui, // Added for RSU IP Interface
                        Ipv4InterfaceContainer & uisr, // Added for RSU IP Interface secondary
                        Ipv4InterfaceContainer & uavi, // UAV IP Interface
                        Ipv4InterfaceContainer & uavisr, // UAV IP Interface secondary
                        double totalTime,
                        int protocol,
                        ...
                        int routingTables)
{
  ...
  SetupRoutingProtocol (c, u, uav);
  AssignIpAddresses (d, i, ud, udsr, ui, uisr, uavd, uavdsr, uavi, uavisr);
  SetupRoutingMessages (c, i, u, ui);
}

// Setting up routing protocol
void
RoutingHelper::SetupRoutingProtocol (NodeContainer & c, NodeContainer & u, NodeContainer & uav) // u is for rsu node
{
  AodvHelper aodv;
  OlsrHelper olsr;
  DsdvHelper dsdv;
  ...
  if (m_protocol < 4)
    {
      ...
      internet.Install (uav); // Installing on UAV nodes
    }
...
void
RoutingHelper::AssignIpAddresses (NetDeviceContainer & d, // vehicle netdevice
                                  Ipv4InterfaceContainer & adhocTxInterfaces, // vehicle ip interface
                                  NetDeviceContainer & ud, // rsu netdevice
                                  NetDeviceContainer & udsr, // rsu secondary netdevice
                                  Ipv4InterfaceContainer & ui, // rsu ip interface
                                  Ipv4InterfaceContainer & uisr, // rsu ip interface secondary
                                  NetDeviceContainer & uavd, // UAV netdevice
                                  NetDeviceContainer & uavdsr, // UAV secondary netdevice
                                  ...
                                  Ipv4InterfaceContainer & uavisr) // uav ip interface secondary
{
  ...
  Ipv4AddressHelper addressAdhocSR;

  // Primary IP address
  addressAdhoc.SetBase ("10.1.0.0", "255.255.0.0");
  adhocTxInterfaces = addressAdhoc.Assign (d); // Vehicle Primary
  ui = addressAdhoc.Assign (ud); // RSU Primary
  uavi = addressAdhoc.Assign (uavd); // UAV Primary

  // Secondary IP address
  addressAdhocSR.SetBase ("20.1.0.0", "255.255.0.0");
  uisr = addressAdhocSR.Assign (udsr); // RSU Secondary
  uavisr = addressAdhocSR.Assign (uavdsr); // UAV Secondary
}

void
RoutingHelper::SetupRoutingMessages (NodeContainer & c, Ipv4InterfaceContainer & adhocTxInterfaces,
                                    NodeContainer & d, Ipv4InterfaceContainer & rsuTxInterfaces)
{
...
private:
  ...
  uint32_t m_nUAVNodes; ///< number of UAV
  ...
  // Net Device for RSU Secondary
  NetDeviceContainer m_rsuTxDevicesSR;

  // Net Device for UAV
  NetDeviceContainer m_uavTxDevices;
  // Net Device for UAV Secondary
  NetDeviceContainer m_uavTxDevicesSR;
  ...
  // Ipv4 interface for RSU secondary
  Ipv4InterfaceContainer m_rsuTxInterfacesSR;

  // Ipv4 interface for UAV
  Ipv4InterfaceContainer m_uavTxInterfaces;
  // Ipv4 interface for UAV secondary
  Ipv4InterfaceContainer m_uavTxInterfacesSR;
...
VanetRoutingExperiment::VanetRoutingExperiment ()
  : m_port (9),
    ...
    m_protocol (2), // set protocol // 2=aodv
    ...
    m_nNodes (156),
    m_TotalSimTime (300.01),
    m_rate ("10240bps"), // default was 2048bps
...
VanetRoutingExperiment::SetupScenario ()
{
  ...
  m_logFile = "low99-ct-unterstrass-1day.filt.7.adj.log";
  m_mobility = 1;
  m_nNodes = 164; // Number of vehicles in the mobility file
  m_TotalSimTime = 10.0;
  m_nodeSpeed = 0;
  m_nodePause = 0;
...
VanetRoutingExperiment::SetupAdhocDevices ()
{
  ...
  YansWifiChannelHelper wifiChannelUAV;
  wifiChannel.SetPropagationDelay ("ns3::ConstantSpeedPropagationDelayModel");
  wifiChannelUAV.SetPropagationDelay ("ns3::ConstantSpeedPropagationDelayModel");
  ...
  wifiChannelUAV.AddPropagationLoss (m_lossModelName, "Frequency", DoubleValue (freq), "HeightAboveZ", DoubleValue (200.0));
  ...
  if (m_fading != 0)
    {
      ...
      wifiChannelUAV.AddPropagationLoss ("ns3::NakagamiPropagationLossModel");
    }

  // the channel for vehicles
  Ptr<YansWifiChannel> channelVehicle = wifiChannel.Create ();
  // The below set of helpers will help us to put together the wifi NICs we want
  YansWifiPhyHelper wifiPhy =  YansWifiPhyHelper::Default ();
  wifiPhy.SetChannel (channelVehicle);
  YansWavePhyHelper wavePhy =  YansWavePhyHelper::Default ();
  wavePhy.SetChannel (channelVehicle);
  
  //the channel for secondary interface for UAV and RSU
  Ptr<YansWifiChannel> channelUAV = wifiChannelUAV.Create ();
  YansWifiPhyHelper wifiPhyUAV =  YansWifiPhyHelper::Default ();
  wifiPhyUAV.SetChannel (channelUAV);
  YansWavePhyHelper wavePhyUAV =  YansWavePhyHelper::Default ();
  wavePhyUAV.SetChannel (channelUAV);
  ...
  wifiPhyUAV.Set ("TxPowerStart",DoubleValue (20.0));
  wifiPhyUAV.Set ("TxPowerEnd", DoubleValue (20.0));

  wavePhy.Set ("TxPowerStart",DoubleValue (m_txp));
  wavePhy.Set ("TxPowerEnd", DoubleValue (m_txp));

  wavePhyUAV.Set ("TxPowerStart",DoubleValue (20.0));
  wavePhyUAV.Set ("TxPowerEnd", DoubleValue (20.0));
  ...
  m_rsuTxDevicesSR = wifi80211p.Install (wifiPhyUAV, wifi80211pMac, m_rsuNodes);
  m_uavTxDevices = wifi80211p.Install (wifiPhy, wifi80211pMac, m_UAVNodes);
  m_uavTxDevicesSR = wifi80211p.Install (wifiPhyUAV, wifi80211pMac, m_UAVNodes);
}
...
VanetRoutingExperiment::SetupAdhocMobilityNodes ()
{
  ...
  uavPositionAlloc->Add (Vector (2163.7263,1941.7648, 200.0));
  uavPositionAlloc->Add (Vector (2059.4635,47.8347, 200.0));
  uavPositionAlloc->Add (Vector (1088.2885,2250.5028, 200.0));
  uavPositionAlloc->Add (Vector (507.6378,1350.3231, 200.0));
  uavMobility.SetMobilityModel ("ns3::ConstantPositionMobilityModel");
  uavMobility.SetPositionAllocator (uavPositionAlloc);
  uavMobility.Install (m_UAVNodes);
...
VanetRoutingExperiment::SetupRoutingMessages ()
{
  m_routingHelper->Install (m_adhocTxNodes,
                            ...
                            m_UAVNodes, // Nodes for UAV
                            m_adhocTxDevices,
                            m_rsuTxDevices, // NetDevice for RSU node
                            m_rsuTxDevicesSR, // NetDevice for RSU Secondary interface
                            m_uavTxDevices, // NetDevice for UAV
                            m_uavTxDevicesSR, // NetDevice for UAV Secondary interface
                            m_adhocTxInterfaces,
                            m_rsuTxInterfaces, // IP interface for RSU node
                            m_rsuTxInterfacesSR, // IP interface for RSU node secondary
                            m_uavTxInterfaces, // IP interface for UAV
                            m_uavTxInterfacesSR, // IP interface for UAV secondary
                            m_TotalSimTime,
                            m_protocol,
                            m_nSinks,
                            m_routingTables);
...
VanetRoutingExperiment::Run ()
{
  ...
  // Added code for netanim
  AnimationInterface anim("FUAV.xml");
  Simulator::Run ();
  Simulator::Destroy ();
}
...
VanetRoutingExperiment::ProcessOutputs ()
{
  ...
  uint32_t totalPacketSend = m_routingHelper->GetRoutingStats ().GetCumulativeTxPkts (); 
  uint32_t totalPacketRcvd = m_routingHelper->GetRoutingStats ().GetCumulativeRxPkts (); 
  double pdr = (double)totalPacketRcvd/totalPacketSend; 
  uint32_t totalEED = m_routingHelper->GetRoutingStats ().GetTotalEED (); 
  double avgEED = (double)totalEED/totalPacketRcvd; 
